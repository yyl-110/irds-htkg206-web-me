...
const onSubmitFormData = async (type: string) => {
  ...
  searchValue.value = '';
  if (Array.isArray(checkedKeys.value)) {
    await ChangeTreeattribute(treeData.value, checkedKeys.value);
    await ChangeTreeattribute(temporaryTreeData.value, checkedKeys.value);
  } else {
    await ChangeTreeattribute(treeData.value, checkedKeys.value.checked);
    await ChangeTreeattribute(temporaryTreeData.value, checkedKeys.value.checked);
  }
  let data = {
    basicId: demandrow.value.basicId,
    type: Customertype.value,
    allocateTree: treeData.value,
    saveStatus: 1,
  };
  if (type == 'save') {
    data.saveStatus = 1;
...
async function saveAllocate(data: any) {
  const res = await AdminApiSystemistribution.saveAllocate(data);
...
function customGetContainer() {
  // 返回自定义挂载节点
  return document.querySelector('.ibution-update');
}
defineExpose({ handleModalAddOrUpdate });
</script>
<template>
  <div class="ibution-update" v-dragModal>
    <a-modal
