...
import DistributeBulletBoxes from './components/form/index.vue';
import DistributeBulletBoxesdetail from './components/detail/index.vue';
...
const DecisionConclusiondetailVisible = ref<boolean>(false);
...
const RefConclusiondetail = ref<any>();
/** 列表请求参数 */
...
const RequirementsatheringList = ref([
  ...
  {
    key: '2',
    title: WeiI18n.t('待分发').value,
    number: '',
    iconType: 'icon-daifenfadingdan',
    Key: 'unAllocateCount',
  },
  {
    key: '3',
...
function disabledibution(row: any) {
  if (row.status === TaskStatus.DISPATCH_PENDING || row.status === TaskStatus.DISTRIBUTION) {
    return false;
  } else {
    return true;
  }
...
async function getListData() {
  ...
  statisticInfo();
  try {
    const res = await AdminApiSystemistribution.getdemandallocateList({
      ...requestParams,
    });
...
async function statisticInfo() {
  try {
    const res = await AdminApiSystemistribution.getibutionstatisticInfo({});
    if (res.data.code == 200) {
...
function RequirementProcessingdetail(data: any) {
  DecisionConclusiondetailVisible.value = true;
  nextTick(() => {
    RefConclusiondetail.value.handleModalAddOrdetail(data);
  });
}
...
async function revoke(row: any) {
  Modal.confirm({
    ...
    async onOk() {
      const res = await AdminApiSystemistribution.retract({ basicId: row.basicId });
      if (res.data.code == 200) {
        getListData();
...
function onClosedetail() {
  DecisionConclusiondetailVisible.value = false;
}
...
<template>
              ...
              <a-table
                ...
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-if="column.dataIndex === 'operation'">
                    <a-button shape="circle" type="link" :disabled="Isiteditable(record) || disabledibution(record)" @click="RequirementProcessing(record)">需求分发</a-button>
                    <a-divider type="vertical" />
                    <a-button shape="circle" type="link" :disabled="Isiteditable(record) || disabledIrevoke(record)" @click="revoke(record)">撤回</a-button>
                    <a-divider type="vertical" />
                    <a-button shape="circle" type="link" @click="RequirementProcessingdetail(record)">详情</a-button>
                  </template>
                </template>
              </a-table>
            </a-tab-pane>
  ...
  <DistributeBulletBoxes :modalVisible="AddUpdateVisible" ref="RefAddUpdate" @onClose="onClose" @RefreshtableData="RefreshtableData" />
  <DistributeBulletBoxesdetail :modalVisible="DecisionConclusiondetailVisible" ref="RefConclusiondetail" @onClose="onClosedetail" @RefreshtableData="getListData" />
  <a-image
    :width="200"
    :style="{ display: 'none' }"
    :preview="{
