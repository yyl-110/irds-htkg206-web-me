...
import IPMTDecisionHandling from './components/form/IPMTDecisionHandling.vue';
import DecisionConclusionModel from './components/form/DecisionConclusion.vue';
...
const IPMTDecisionVisible = ref<boolean>(false);
const DecisionConclusionVisible = ref<boolean>(false);
const loading = ref<boolean>(false);
const tableHeight = ref(window.innerHeight - 450);
const datasource = ref([]); // 系列设计列表数据
const RefAddUpdate = ref<any>();
const RefIPMTDecision = ref<any>();
const RefConclusion = ref<any>();
/** 列表请求参数 */
...
const columns = ref<TableColumnType[]>([
  ...
  {
    title: WeiI18n.$t('RAT处理时间'),
    dataIndex: 'decisionConclusionTime',
    key: 'decisionConclusionTime',
    align: 'center',
    resizable: true,
    sorter: (a: any, b: any) => sortermethod(a.decisionConclusionTime, b.decisionConclusionTime),
    width: 130,
  },
  {
...
// RME已分析/待处理状态可以需求处理
function disabledprocess(row: any) {
  return row.status !== TaskStatus.RME_ANALYZED;
}
// RAT待分析和RAT分析中可以决策分析
function disabledAnalysis(row: any) {
  if (row.status === TaskStatus.RAT_NOT_ANALYZED) {
    return false;
  } else {
    return true;
  }
...
// 待IPMT待决策可以IPMT决策
function disabledIPMTDecision(row: any) {
  return row.status !== TaskStatus.IPMT_PD;
}
// RAT分析完成和IPMT已决策可以决策结论
function disabledDecisionConclusion(row: any) {
  if (row.status === TaskStatus.RAT_AC || row.status === TaskStatus.IPMT_DEC) {
    return false;
  } else {
    return true;
  }
...
function disabledIrevoke(row: any) {
  if (row.status === TaskStatus.RAT_NOT_ANALYZED || row.status === TaskStatus.RAT_ANALYZING || row.status === TaskStatus.IPMT_DEC || row.status === TaskStatus.DISPATCH_PENDING) {
    return false;
  } else {
    return true;
  }
...
async function getListData() {
  ...
  statisticInfo();
  try {
    const res = await AdminApiSystemDemandanalyze.getdemandanalyzeList({
      ...requestParams,
    });
...
async function statisticInfo() {
  try {
    const res = await AdminApiSystemDemandanalyze.getanalyzestatisticInfo({});
    if (res.data.code == 200) {
...
function RequirementProcessing(row: any) {
  AddUpdateVisible.value = true;
  nextTick(() => {
    RefAddUpdate.value?.handleModalAddOrUpdate(row, 'edit');
  });
}
async function DecisionAnalysis(row: any) {
  Modal.confirm({
    title: `确定要发起决策分析吗？`,
    okText: WeiI18n.t('确定').value,
    cancelText: WeiI18n.t('取消').value,
    async onOk() {
      const res = await AdminApiSystemDemandanalyze.initiateDecisionAnalyze({ basicId: row.basicId });
      if (res.data.code == 200) {
        getListData();
...
function IPMTDecision(row: any) {
  IPMTDecisionVisible.value = true;
  nextTick(() => {
    RefIPMTDecision.value.handleModalAddOrUpdate(row);
  });
}
function CallConclusionComponent(data: any, type: string) {
  DecisionConclusionVisible.value = true;
  nextTick(() => {
    RefConclusion.value.handleModalAddOrUpdate(data, type);
  });
}
function DecisionConclusion(row: any, type: string) {
  if (type == 'single') {
    CallConclusionComponent(row, type);
  } else {
    let satisfyarr: any = [];
    let dissatisfied: any = [];
    selectedRowList.value.forEach((item: any) => {
      if (item.status === TaskStatus.RAT_AC || item.status === TaskStatus.IPMT_DEC) {
        satisfyarr.push(item.basicId);
      } else {
        dissatisfied.push(item.demandNum);
      }
    });
    if (dissatisfied.length > 0) {
      Batchdecisionconclusion(satisfyarr, type, dissatisfied);
    } else if (satisfyarr.length > 0) {
      CallConclusionComponent(satisfyarr, type);
    }
  }
}
function Batchdecisionconclusion(satisfyarr: any, type: string, dissatisfied: any) {
  if (satisfyarr.length == 0) {
    message.warning('所选数据未进行决策分析');
    return;
  }
  Modal.confirm({
    title: `需求编号${dissatisfied.join('、')}未处理，继续提交系统只将处理完成数据进行提交，是否继续？`,
    okText: WeiI18n.t('确定').value,
    cancelText: WeiI18n.t('取消').value,
    width: 500,
    async onOk() {
...
async function revoke(row: any) {
  Modal.confirm({
    ...
    cancelText: WeiI18n.t('取消').value,
    async onOk() {
      const res = await AdminApiSystemDemandanalyze.retract({ basicId: row.basicId });
      if (res.data.code == 200) {
        getListData();
        message.success('操作成功');
      }
...
function onCloseIPMTDecision() {
  IPMTDecisionVisible.value = false;
}
function onCloseConclusion() {
  DecisionConclusionVisible.value = false;
}
...
<template>
          ...
          <a-tabs v-model:activeKey="taskIndex" @tab-click="getTasks" class="body_box">
            <a-tab-pane v-for="item in tabLabelList" :key="item.label" style="border: 0px">
              <a-form style="margin-bottom: 10px" layout="inline" :label-col="{ style: { width: '100px' } }" :model="requestParams">
                ...
                <a-form-item>
                  <a-button type="primary" :disabled="selectedRowkeys.length === 0" @click="DecisionConclusion(selectedRowList.value, 'batch')"
                    ><EpcIcon type="icon-jielun-copy" style="font-size: 16px" /> {{ $t('批量决策结论') }}</a-button
                  >
                </a-form-item>
              </a-form>
              ...
              <a-table
                ...
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-if="column.dataIndex === 'operation'">
                    <a-button shape="circle" :disabled="Isiteditable(record) || disabledprocess(record)" @click="RequirementProcessing(record)" type="link">需求处理</a-button>
                    <a-divider type="vertical" />
                    <a-button shape="circle" :disabled="Isiteditable(record) || disabledAnalysis(record)" @click="DecisionAnalysis(record)" type="link">决策分析</a-button>
                    <a-divider type="vertical" />
                    <a-button shape="circle" type="link" :disabled="Isiteditable(record) || disabledIPMTDecision(record)" @click="IPMTDecision(record)">IPMT决策</a-button>
                    <a-divider type="vertical" />
                    <a-button shape="circle" type="link" :disabled="Isiteditable(record) || disabledDecisionConclusion(record)" @click="DecisionConclusion(record, 'single')"
                      >决策结论</a-button
                    >
                    <a-divider type="vertical" />
  ...
  <IPMTDecisionHandling :modalVisible="IPMTDecisionVisible" ref="RefIPMTDecision" @onClose="onCloseIPMTDecision" @RefreshtableData="RefreshtableData" />
  <DecisionConclusionModel :modalVisible="DecisionConclusionVisible" ref="RefConclusion" @onClose="onCloseConclusion" @RefreshtableData="RefreshtableData" />
  <a-image
    :width="200"
    :style="{ display: 'none' }"
    :preview="{
