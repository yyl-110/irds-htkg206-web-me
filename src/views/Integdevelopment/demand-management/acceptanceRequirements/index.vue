...
import acceptanceccomponent from './components/form/index.vue';
...
const acceptanceccomponentVisible = ref<boolean>(false);
const loading = ref<boolean>(false);
const datasource = ref([]);
const tableHeight = ref(window.innerHeight - 450);
const acceptanceccomponentRef = ref<any>();
/** 列表请求参数 */
const requestParams = reactive<any>(new ProdDemandacceptanceRequestDTOModel());
...
async function getListData() {
  await GettingProgress();
  console.log(Progresstotal.value, Progresstotal.value);
  ...
  statisticInfo();
  try {
    const res = await AdminApiSystemacceptance.getdemandacceptList({
      ...requestParams,
    });
...
// 获取设计列表数据
async function GettingProgress() {
  let data = {
    pageNo: 1,
    pageSize: 10,
    demandNum: '',
  ...
  if (res.data.code == 200) {
    Progresstotal.value = Number(res.data.data.total) || 0;
  }
}
...
async function statisticInfo() {
  try {
    const res = await AdminApiSystemacceptance.getacceptsticInfo({});
    if (res.data.code == 200) {
...
function acceptance(row: any) {
  acceptanceccomponentVisible.value = true;
  nextTick(() => {
    acceptanceccomponentRef.value.handleModalacceptancec(row);
  });
}
...
<template>
              ...
              <a-table
                ...
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-if="column.dataIndex === 'operation'">
                    <a-button shape="circle" :disabled="Isiteditable(record) || disabledacceptance(record)" @click="acceptance(record)" type="link">需求验收</a-button>
                  </template>
                </template>
              </a-table>
            </a-tab-pane>
  ...
  <acceptanceccomponent
    :modalVisible="acceptanceccomponentVisible"
    ref="acceptanceccomponentRef"
    @onClose="acceptanceccomponentVisible = false"
