...
async function Changeform() {
  ...
  formData.value.changeReason = Upgradeform.changeReason;
  formData.value.changeProofFileIds = arr;
  let data: any = { ...formData.value };
  data.firstTechnology = firstTechnology;
  data.secondTechnology = secondTechnology;
  const res = await AdminApiSystemchange.originalChange(data);
  if (res.data.code == 200) {
...
<template>
  ...
  <a-modal style="width: 90%" v-model:visible="visible" :confirm-loading="$isPending()" :mask-closable="false" :title="visibletitle" @cancel="cancel">
    <a-collapse class="custom-collapse form-container" v-model:activeKey="activeKey" :bordered="false">
      <a-collapse-panel key="1" header="原始需求">
          ...
          <a-spin :spinning="loading" tip="加载中...">
            <div style="display: flex">
              ...
              <div style="width: 60%">
                ...
                <a-form-item label="应用场景描述" name="sceneDescription" :rules="[{ required: Departmentalverification(), message: '请输入需求标题' }]">
              ...
              <div style="width: 30%; margin-left: 5%">
                ...
                <a-form-item label="产品平台" name="platform">
                  <a-select allowClear v-model:value="formData.platform" placeholder="请选择产品平台" @change="PlatformChange">
