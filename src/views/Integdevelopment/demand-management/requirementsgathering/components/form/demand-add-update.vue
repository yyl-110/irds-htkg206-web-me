...
const visibletitle = ref<string>('原始需求预分析');
...
async function Saveandsubmitform() {
  ...
  const res = await AdminApiSystemDemandcollect.saveMaster(data);
  if (res.data.code == 200) {
            ...
            <div style="display: flex">
              ...
              <div style="width: 60%">
                <a-form-item label="需求标题" name="title">
                  <a-textarea class="textarea" disabled v-model:value="beginningformData.demandTitle" :rows="2" maxlength="30" />
                  <h5 class="conter_title">提示：提高车辆座椅舒适性</h5>
                </a-form-item>
                <a-form-item label="原始需求描述" name="description">
                  <CkeditorPlugin :disabled="true" ref="ckeditorRef1" height="200" />
                </a-form-item>
                <a-form-item label="客户应用场景描述">
