...
async function updatePram(record: any) {
  modalTitle.value = '编辑' + record.productName + '销量预算';
...
function customGetContainer() {
  // 返回自定义挂载节点
  return document.querySelector('.Xlyc');
}
          ...
          <div class="year-inputs-grid">
            <div v-for="i in 5" :key="i" class="year-input-item">
              <a-form-item :label="String(Number(addYear) + i - 1)" :name="`year${i}`">
                <a-input-number v-model:value="formData[`year${i}`]" min="1" :placeholder="`请输入${String(Number(addYear) + i - 1)}年数据`" style="width: 200px" />
              </a-form-item>
            </div>
          </div>
