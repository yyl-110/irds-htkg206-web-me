...
export default {
  ...
  data() {
    return {
      ...
      userTaskForm: {},
      mockData: ['#{assignee}', '#{assignee1}', '#{assignee2}', '#{assignee3}','#{assignee4}'],
    };
  },
  mounted() {
    window.bpmnInstances.modeling.updateProperties(window.bpmnInstances.bpmnElement, { assignee: '#{assignee}' });
  },
  watch: {
    id: {
      immediate: true,
