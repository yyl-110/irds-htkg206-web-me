  ...
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="small" border>
      ...
      <el-table-column label="操作" width="90px">
        <template v-slot="{ row, $index }">
          <el-button link type="" @click="openListenerForm(row, $index)"
            >编辑</el-button
          >
    ...
    >
      ...
      >
        ...
        <el-table-column label="监听类型" align="left" prop="type">
        </el-table-column>
        <el-table-column label="事件类型" align="left" prop="eventType" />
        <el-table-column
          label="执行内容"
          align="left"
          prop="value"
      ...
      <div class="botBtn">
        <el-button type="primary" @click="builtInMonitor = false"
          >取消</el-button
        >
        <el-button type="primary" @click="saveListener">保存</el-button>
      </div>
    </el-drawer>
    ...
    >
      ...
      >
        ...
        <el-table-column label="操作" width="100px">
          <template v-slot="{ row, $index }">
            <el-button link type="" @click="openListenerFieldForm(row, $index)"
              >编辑</el-button
            >
            <el-divider direction="vertical" />
            <el-button
...
export default {
  ...
  methods: {
    /** 查询流程监听列表 */
    getList() {
      listListener(this.queryParams).then((res) => {
        this.listenerList = res.rows.filter((v) => {
          return v.type === "执行监听";
        });
    ...
    handleSelectionChange(val) {
      //console.log(val, ' dddddddddddval');
      this.listenerListData = val;
    },
    // 保存监听
    saveListener() {
      //console.log('保存接口');
      this.elementListenersList.push(...this.listenerListData);
      this.builtInMonitor = false;
      //console.log(this.elementListenersList, 'this.elementListenersList');
    },
    // 内置监听
    openListenerMonitor() {
      this.getList();
      this.builtInMonitor = true;
    },
    resetListenersList() {
