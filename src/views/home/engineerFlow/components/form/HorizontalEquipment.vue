      ...
      <a-row class="elrowheight">
        ...
        <div class="textar-wrap">
          ...
          <div class="upload-file-wrap">
            <div class="file-list" v-for="(item, index) in uploadPlaceData" :key="index">
              
              <div class="file-dec">
                <div class="tit">{{ item.originalFilename }}</div>
                <div class="number">
                  <span>编号：{{ item.docNumber }}</span
                  ><a class="hover-link" type="primary" :href="item.url" target="_blank">连接：{{ item.url }}</a>
                </div>
              </div>
            </div>
          </div>
          <div class="textar-wrap">
            <div class="sub-title">车下设备布置：</div>
          </div>
          <div class="upload-file-wrap">
            <div class="file-list" v-for="(item, index) in equipmentUpload" :key="index">
          ...
          <div class="textar-wrap">
            ...
            <a-textarea   v-model:value="props.equipmentString" :rows="4" type="textarea" placeholder="车下设备布置描述" disabled />
          </div>
      </a-row>
    </div>
  </div>
