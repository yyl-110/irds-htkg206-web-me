...
function canAssignParameterToTargetNode(node: any) {
  if (!node?.key) {
    return false;
  }
...
async function loadAssignCategoryTree(menuIdValue: string) {
  ...
  try {
    const query = {
      ...treeParameterParams,
      menuId: Number(menuIdValue),
      pageNo: 0,
      pageSize: 0,
      currentPage: 0,
      numberPage: 0,
    };
    const res = await AdminApiSystemParameter.getParameterTree(query);
...
async function confirmAssignCategory() {
  ...
  if (!canAssignParameterToTargetNode(targetNode)) {
  ...
  const parameterIds = (selectedRowList.value || []).map((row: any) => row.id).filter((id: any) => id != null && id !== '');
  if (!parameterIds.length) {
    message.warning('未找到有效的参数记录');
    return;
  }
  assignCategorySubmitLoading.value = true;
  try {
    const res = await AdminApiSystemParameter.assignParametersToCategory({
      parameterIds,
    ...
    if (res?.data?.code === 0 || res?.data?.code === 200) {
      ...
      if (menuId.value === assignCategoryActiveMenuId.value) {
        await getListData('change', assignCategorySelectedKey.value);
      }
    } else {
