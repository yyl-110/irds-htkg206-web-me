...
//初始化数据
async function infoReload(modelTypeIdS: string, productSericeName: string, productSericeId: string, row: any) {
...
function tabDesignClick(tab: any) {
  if (tab === 'second') {
    ...
    nextTick(() => {
      productDeliverylistRef.value?.reloadTaskInfosss(productId.value, 'edit');
    });
  }
  activeKey.value = tab;
}
...
async function saveDesign() {
  ...
  const res = await DesignApiInfo.updateProjectApi({ ...requestInfoParams });
  ...
  <a-modal v-model:visible="itemDialog" title="选择项目" draggable width="1000px">
    <div style="width: 100%; background: #fff; margin-top: 4px">
      <div class="grid-content" style="padding: 0; background: #fff; border: none">
        ...
        <a-button type="primary" @click="itemSearch">搜索</a-button>
      </div>
      <!--    表格处 -->
      <a-table
        v-if="!loadingPro"
  ...
  <a-modal v-model:visible="memberDialogDesign" :title="'添加' + addMemberTit" draggable width="1000px">
    <div style="width: 100%; background: #fff; margin-top: 4px">
      <div class="grid-content" style="padding: 0; background: #fff; border: none">
        ...
        <a-button type="primary" @click="turnToTask" style="margin-left: 10px"
          ><el-icon><Search /></el-icon>&nbsp;搜索</a-button
        >
      </div>
      <!--    表格处 -->
      <a-table
