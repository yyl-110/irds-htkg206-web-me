...
function setupGantt() {
  ...
  const ganttColumns = props.columns.map(col => {
    ...
    //actualEndTime
    if (col.dataIndex === 'actualEndTime') {
      column.template = function(task) {
        if (task.taskStatus === 2 && task.actualEndTime) {
          return task.actualEndTime;
        } else {
          return '/';
        }
      };
