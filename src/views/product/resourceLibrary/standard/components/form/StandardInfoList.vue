...
import StandardAddOrUpdate from '../modal/StandardAddOrUpdate.vue';
...
const standardBoxRef = ref<any>();
function standardBoxStyle(): Record<string, string> {
  ...
  window.addEventListener('resize', () => {
    ...
    if (currentPixelRatio !== lastPixelRatio) {
      ...
      standardBoxRef.value.style.minHeight = `${pageHeight}px`;
    }
...
<template>
  ...
  <StandardAddOrUpdate ref="addOrUpdate" :modal-visible="AddVisible" @modal-init="modalInit" @on-close="AddVisible = false" />
  ...
  <a-drawer v-model:visible="pageFlagDrawer" class="module-detail-drawer" title="模块详情" placement="right" :closable="false" width="800">
    <div ref="standardBoxRef" class="module-detail-drawer-inner px-[16px]" :style="standardBoxStyle()">
      <a-tabs v-model:activeKey="parmType" class="module-detail-standard-tabs" @change="toParm" :animated="false">
        <a-tab-pane :key="'viz'" tab="可视化">
          <div class="standardPage_style module-viz-tab">
        ...
        <a-tab-pane :key="5" tab="历史文档">
          <div class="standardPage_style history-doc-table-wrap">
            <a-table
              :scroll="{ x: 1200 }"
              row-key="id"
              :loading="loading"
...
:deep(.module-detail-standard-tabs.ant-tabs) {
  flex: 1 1 0;
  min-height: 0;
  display: flex;
  flex-direction: column;
...
:deep(.module-detail-standard-tabs .ant-tabs-nav) {
  flex: 0 0 auto;
  margin-bottom: 0;
}

:deep(.module-detail-standard-tabs .ant-tabs-content-holder) {
  flex: 1 1 0;
  min-height: 0;
  overflow: hidden;
}

:deep(.module-detail-standard-tabs .ant-tabs-content) {
  height: 100%;
  overflow: hidden;
}

:deep(.module-detail-standard-tabs .ant-tabs-tabpane) {
  height: 100%;
  min-height: 0;
  overflow-x: hidden;
  overflow-y: auto;
