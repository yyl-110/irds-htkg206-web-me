...
import { ArrowLeftOutlined, DownOutlined, ExportOutlined, FileOutlined } from '@ant-design/icons-vue';
...
function initFromRoute() {
  const nextTemplateId = String(route.query.templateId ?? '').trim();
  const nextFileId = String(route.query.fileId ?? '').trim();
  if (!nextTemplateId) {
    message.warning('缺少模板ID');
    goBack();
    return false;
...
<template>
  <div class="report-setting-page">
    <div class="page-body">
      <a-spin :spinning="parseLoading" tip="加载中...">
