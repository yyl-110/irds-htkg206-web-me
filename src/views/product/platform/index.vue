...
const projectManagerPrient = ref<any>(null);
...
function convertToTreeNodes(data: any[]): any[] {
  return data.map(item => {
    ...
    return {
      ...
      managerUserList: item.managerUserList || [],
...
async function selectNode(selectedKeys: any) {
  ...
  } else if (activKey.value === '4') {
    if (currentNodeLevel.value == 2) {
      projectManagerPrient.value?.reloadTableParameter(selectedKeys);
    } else {
      activKey.value = '3';
      projectManager.value?.reloadTableParameter(selectedKeys);
    }
...
async function handleTabChange(key: string) {
  ...
  if (activKey.value === '1') {
    ...
    projectManagerPrient.value?.reloadTableParameter(currentNodeData.value);
  }
}
...
async function exportParameterInfo() {
  try {
    const res = await AdminApiSystemProduct.exportProjectInfoByProIdAll({});
    const fileName = '平台项目信息.xlsx';
...
async function getNodeUpdateData(selectedKeys: any) {
  ...
  let arr: any = [];
  let arrId: any = [];
  if (selectedKeys.managerUserList && selectedKeys.managerUserList.length > 0) {
    selectedKeys.managerUserList.forEach((item: any) => {
...
async function submitTreeData(nodeList: any, selectedKeys: any) {
  ...
  treeRequestParams.managerIds = nodeList.managerIds;
  if (nodeList.managerIds == '') {
    treeRequestParams.managerIds = [];
  }
  // 这里可以根据需要实现提交树节点数据的逻辑
...
async function editTreeData(nodeList: any, selectedKeys: any) {
  ...
  nextTick(() => {
    ...
    treePage.value?.initializationassignment(rootNode);
  });
...
async function handleUserBrowse() {
  ...
  let Ids: any = [];
  debugger;
  if (treenode.value && treenode.value.managerUserList.length > 0) {
    treenode.value.managerUserList.forEach((item: any) => {
      Ids.push(`${item.id}`);
    });
...
function Personnelselection(item: any) {
  personnel.value = item;
  handleUserBrowse();
}
async function handleModalConfirm(item: any) {
  grpuVisible.value = false;
  let arr: any = [];
  groupData.value.forEach((ite: any) => {
    item.forEach((item: any) => {
      if (ite.id === item) {
        arr.push(ite.nickname);
      }
    });
  });
  nextTick(() => {
    treePage.value?.DisplayPersonnel(personnel.value.key, arr.join(','), item);
  });
}
        ...
        <div v-if="currentNodeLevel === 1" class="level-1-content">
          <a-card>
            <a-link
              ...
              >超级构型结构</a-link
            ><a-link
              style="cursor: pointer; color: #165dff; text-decoration: none; margin-left: 10px"
              @mouseenter="e => (e.target.style.textDecoration = 'underline')"
              @mouseleave="e => (e.target.style.textDecoration = 'none')"
