...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function customGetContainer() {
      // 返回自定义挂载节点
      return document.querySelector('.updateGbomParamter');
    }

    function customGetContainerModal() {
      // 返回自定义挂载节点
      return document.querySelector('.updateGbomParamterModal');
    }
    ...
    async function updateParaneterInfo() {
      ...
      context.emit('updateGbomParaneterInfo', updatePlatformParameter);
    }
...
<template>
  <div class="updateGbomParamter" v-dragModal>
    <a-modal
  ...
  <div class="updateGbomParamterModal" v-dragModal>
    <a-modal
      v-model:visible="listVisible"
      style="width: 30%"
      :style="{ top: '5%' }"
      :title="$t('列表参数配置')"
      :getContainer="customGetContainerModal"
      :mask-closable="false"
