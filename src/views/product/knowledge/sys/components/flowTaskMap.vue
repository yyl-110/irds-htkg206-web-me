<template>
  <a-layout class="layout">
    <a-layout-sider class="elAside" theme="light" :width="224">
      <a-input
        class="search"
        ...
        <div style="padding: 0 6px 10px 10px; margin-top: -8px">
          <div class="sideContent-data-top">
            ...
            <div class="sideContent-data-top-right">
              ...
              <span v-if="item.enableModify" class="sideContent-data-top-right-text" @click.stop="mapEdit(item)">编辑</span>
              <span v-if="item.enableDelete" style="color: #155bd4; cursor: pointer" @click.stop="mapDetele(item)">删除</span>
            </div>
          </div>
  ...
  <DetailDialog :dialogVisible="dialogVisible" :objData="objData" @getList="getMainData" @closeDiaDetail="closeDiaDetail"></DetailDialog>
</template>

<script setup>
import { ref, watch, defineProps, nextTick, computed, onActivated } from 'vue';
import { message, Modal } from 'ant-design-vue';
import { PlusOutlined, SearchOutlined, PlayCircleOutlined, PauseCircleOutlined, CheckCircleOutlined, EnvironmentOutlined } from '@ant-design/icons-vue';
import { useRouter } from 'vue-router';
// 接口导入保持不变
import { saveLookFileLog, getMapNodeByLevel, getEchartsDataByType, getHistoryList, webFileList } from '@/api/knowledgeBaseApply/index.js';
import { getPdfPreviewPath, doCollectFile, updateKldCounting, taskMapData, modifyInitMap, removeMap, personalMapList } from '@/api/knowledgeBaseManagment';
...
const getMapList = () => {
  ...
  taskMapData(params).then(res => {
    if (res && res.code === '0') {
      sideData.value = [];
      sideData.value = res.data.result;
...
const getMainData = () => {
  ...
  personalMapList(params).then(res => {
    if (res && res.code === '0') {
      ...
      page.value.pageCount = res.data.rowCount;
    } else {
      message.error(res.msg);
    }
  });
...
const mapDetail = item => {
  ...
  objData.value = item;
  if (item.viewed) {
    router.push({ path: '/knowledgeData/createTaskMap_index', query: { flag: '2' } });
  } else {
...
const mapEdit = item => {
  ...
  modifyInitMap(params).then(res => {
    if (res && res.code === '0') {
      store.commit('SET_MAPEDITDATA', res.data);
      router.push({ path: '/knowledgeBaseManagment/createTaskMap' });
    } else {
      message.error(res.msg);
    }
    ...
    .titleText {
      ...
      .iconStyle {
        ...
        .plus {
          ...
          cursor: pointer;
          font-size: 13px;
          border-radius: 50%; /* 如果原有图标是圆形的可以加上这个 */
        }
      }
    }
    .sideContent {
