...
import { createDefaultPage1ParameterList, type Page1ParameterItem } from './parameterDefaults';

export type { Page1ParameterItem };

export function mergeSavedParamsIntoList(
  list: Page1ParameterItem[],
...
export async function loadPage1PageParameters(
  pageId: string,
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Promise<Page1ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultPage1ParameterList(pageKey);
