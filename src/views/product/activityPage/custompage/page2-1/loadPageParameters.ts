...
import { createDefaultPage2_1ParameterList, type Page2_1ParameterItem } from './parameterDefaults';

export type { Page2_1ParameterItem };
...
export async function loadPage2_1PageParameters(
  pageId: string,
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Promise<Page2_1ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultPage2_1ParameterList(pageKey);
