...
import { createDefaultZt1_532CParameterList, type Zt1_532CParameterItem } from './parameterDefaults';

export type { Zt1_532CParameterItem };

async function applyActivityParameterIds(pageId: string, list: Zt1_532CParameterItem[]): Promise<Zt1_532CParameterItem[]> {
...
export async function loadZt1_532CPageParameters(pageId: string): Promise<Zt1_532CParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultZt1_532CParameterList(pageKey);
