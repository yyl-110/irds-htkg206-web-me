<template>
  <div>
    <div class="layout-wrapper" style="padding: 0 10px; background-color: #ffffff">
      ...
      <div class="selectBox">
        <a-button type="primary" class="btnSty" style="margin-left: 5px; margin-top: 10px; margin-right: 30px" @click="initData">
          更新数据
        </a-button>
        <a-button type="primary" class="btnSty" style="margin-left: 20px; margin-top: 10px" @click="assembleModuleByTemplate">
          创建并装配
        </a-button>
        <a-button type="primary" class="btnSty" style="margin-left: 20px; margin-top: 10px" @click="setPartParamInAssembly">
          再生模型
        </a-button>
        <a-button type="primary" class="btnSty" style="margin-left: 20px; margin-top: 10px" @click="assembleModuleByTemplateA">
          装配
        </a-button>
          ...
          <div style="float: left">
            <a-form-item label="机箱机柜集成文件导出：" :label-col="{ style: { width: '150px' } }">
              <a-input
                ...
                style="width: 300px"
                allow-clear
                @input="setSaveBtnEnable(parameterTempList[1].inputOrOutput, parameterTempList[1].parameterId, $event.target?.value)" />
              <a-button type="primary" style="margin-left: 10px; margin-right: 10px" @click="exportDataToFile">导出</a-button>
...
import {
  BASE_VARIANT_CONFIG,
  cloneParameterList,
  initCustomizedProcessPage7Data5_3,
...
import {
  ...
  formatCategoryLabel,
  isInputEditableField,
} from './Process7-page5-3/tableColumns';

defineOptions({ name: 'customizedProcess7-page5-3' });
...
const variantConfig = BASE_VARIANT_CONFIG;
