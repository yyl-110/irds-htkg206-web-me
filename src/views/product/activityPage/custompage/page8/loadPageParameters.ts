...
import { createDefaultPage8ParameterList, type Page8ParameterItem } from './parameterDefaults';

export type { Page8ParameterItem };
...
async function applyActivityParameterIds(pageId: string, list: Page8ParameterItem[]): Promise<Page8ParameterItem[]> {
...
export async function loadPage8PageParameters(
  pageId: string,
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Promise<Page8ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultPage8ParameterList(pageKey);
