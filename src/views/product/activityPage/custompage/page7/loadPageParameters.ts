...
import { createDefaultPage7ParameterList, type Page7ParameterItem } from './parameterDefaults';

export type { Page7ParameterItem };
...
async function applyActivityParameterIds(pageId: string, list: Page7ParameterItem[]): Promise<Page7ParameterItem[]> {
...
export async function loadPage7PageParameters(
  pageId: string,
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Promise<Page7ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultPage7ParameterList(pageKey);
