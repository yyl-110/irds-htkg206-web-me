...
import { createDefaultPage2ParameterList, type Page2ParameterItem } from './parameterDefaults';

export type { Page2ParameterItem };
...
export async function loadPage2PageParameters(
  pageId: string,
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Promise<Page2ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultPage2ParameterList(pageKey);
