...
export const INNER_SKIN_DESIGN_TABLE_COLUMNS: DesignAntColumn[] = [
  ...
  { title: '内径', dataIndex: 'p11', key: 'p11', width: 80, align: 'left', cellMode: 'number' },
...
export const INNER_SKIN_DESIGN_COLUMN_MAP = new Map(
  INNER_SKIN_DESIGN_TABLE_COLUMNS.map(col => [String(col.dataIndex), col]),
);

export const FORM_LEFT_FIELDS = [
  { label: '保温层材质：', index: 0 },
  { label: '保温层厚度：', index: 1 },
  { label: '内蒙皮外径(mm)：', index: 6 },
];

export const FORM_RIGHT_FIELDS = [
  { label: '加热膜材料：', index: 2 },
  { label: '加热膜厚度：', index: 3 },
  { label: '加热膜铺放位置：', index: 4 },
];
