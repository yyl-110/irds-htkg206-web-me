...
import { createDefaultFs151_1_4ParameterList, type Fs151_1_4ParameterItem } from './parameterDefaults';

export type { Fs151_1_4ParameterItem };

async function applyActivityParameterIds(pageId: string, list: Fs151_1_4ParameterItem[]): Promise<Fs151_1_4ParameterItem[]> {
...
export async function loadFs151_1_4PageParameters(pageId: string): Promise<Fs151_1_4ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultFs151_1_4ParameterList(pageKey);
  list = await applyActivityParameterIds(pageKey, list);
  return list;
}

export function extractFs151_1_4SaveParamValues(list: Fs151_1_4ParameterItem[]) {
