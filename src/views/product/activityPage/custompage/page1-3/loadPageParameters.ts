...
import { createDefaultPage1_3ParameterList, type Page1_3ParameterItem } from './parameterDefaults';

export type { Page1_3ParameterItem };

export function mergeSavedParamsIntoList(
  list: Page1_3ParameterItem[],
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Page1_3ParameterItem[] {
...
export async function loadPage1_3PageParameters(
  pageId: string,
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Promise<Page1_3ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultPage1_3ParameterList(pageKey);
