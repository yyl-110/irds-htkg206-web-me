...
import { createDefaultFs151_1_1KParameterList, type Fs151_1_1KParameterItem } from './parameterDefaults';

export type { Fs151_1_1KParameterItem };

async function applyActivityParameterIds(pageId: string, list: Fs151_1_1KParameterItem[]): Promise<Fs151_1_1KParameterItem[]> {
...
export async function loadFs151_1_1KPageParameters(pageId: string): Promise<Fs151_1_1KParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultFs151_1_1KParameterList(pageKey);
  list = await applyActivityParameterIds(pageKey, list);
  return list;
}

export function extractFs151_1_1KSaveParamValues(list: Fs151_1_1KParameterItem[]) {
