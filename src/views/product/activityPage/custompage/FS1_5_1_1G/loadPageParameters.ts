...
import { createDefaultFs151_1GParameterList, type Fs151_1GParameterItem } from './parameterDefaults';

export type { Fs151_1GParameterItem };

async function applyActivityParameterIds(pageId: string, list: Fs151_1GParameterItem[]): Promise<Fs151_1GParameterItem[]> {
...
export async function loadFs151_1GPageParameters(pageId: string): Promise<Fs151_1GParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultFs151_1GParameterList(pageKey);
  list = await applyActivityParameterIds(pageKey, list);
  return list;
}

export function extractFs151_1GSaveParamValues(list: Fs151_1GParameterItem[]) {
