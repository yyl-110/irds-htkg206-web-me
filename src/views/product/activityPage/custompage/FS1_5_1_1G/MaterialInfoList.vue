...
import MaterialAddOrUpdate from '@/views/product/resourceLibrary/material/components/modal/MaterialAddOrUpdate.vue';
import InitiateaProcess from '@/views/product/module/components/modal/InitiateaProcess.vue';
import applicationModule from '@/views/product/module/components/modal/applicationModule.vue';
import parametricdesign from '@/views/product/module/components/modal/parametricdesign.vue';
...
import { GlobalQueryPara10Cell, useGlobalQuery } from '@/views/product/module/composables/useGlobalQuery';
...
/** 选中行 para10 是否为设计中或者编制中 */
function isPara10DraftStatus(record?: any) {
  return String(record?.para10 ?? '').trim() === '设计中' || String(record?.para10 ?? '').trim() === '编制中';
}

/** 是否可发起流程：仅选中一条且 para10 为设计中或者编制中*/
...
async function argsMx(row: any) {
  ...
  if (row.length > 0) {
    ...
    const rec = row[0];
    const displayName = rec?.para3 != null && String(rec.para3).trim() !== '' ? String(rec.para3) : String(rec?.para1 ?? '');
...
function dropdownAction(type: number) {
  if (type == 1) {
    ...
    if (canInitiateProcess.value) {
      if (selectModelList.value[0]?.para10?.trim() === '设计中') {
        processMethod();
      } else if (selectModelList.value[0]?.para10?.trim() === '编制中') {
        router.push({
          name: 'BpmProcessInstanceDetail',
          query: {
            id: selectModelList.value[0]?.para50,
          },
        });
      }
    } else {
...
const materialBoxRef = ref<any>();
function materialBoxStyle(): Record<string, string> {
  ...
  window.addEventListener('resize', () => {
    ...
    if (currentPixelRatio !== lastPixelRatio) {
      ...
      materialBoxRef.value.style.minHeight = `${pageHeight}px`;
    }
...
function resolvePropertyDataIndex(item: { propertyName?: string; dataProp?: string }) {
...
function buildPickerConfirmColumns() {
  ...
  return resData
    .filter((item: any) => item.propertyName && item.dataProp)
    .map((item: any) => {
      const dataIndex = resolvePropertyDataIndex(item);
      return {
...
defineExpose({ initData, selectAllModuleInfo, getPickerConfirmPayload, buildPickerConfirmColumns });
</script>

<template>
  <div class="module-body" :class="pickerMode ? 'module-body--picker' : ''">
    ...
    <div class="module-info-table-wrap" style="width: 100%">
      <a-card class="calc-table-card">
        <a-table
          ...
          :scroll="{ x: moduleTableScrollX, y: pickerMode ? '420px' : undefined }"
  ...
  <a-drawer
    ...
    :closable="false"
    width="800">
    <div ref="materialBoxRef" class="module-detail-drawer-inner px-[16px]" :style="materialBoxStyle()">
      <a-tabs v-model:activeKey="parmType" class="module-detail-material-tabs" @change="toParm" :animated="false">
        <a-tab-pane :key="'viz'" tab="可视化">
          <div class="materialPage_style module-viz-tab">
        ...
        <a-tab-pane :key="5" tab="历史文档">
          <div class="materialPage_style history-doc-table-wrap">
            <a-table
              :scroll="{ x: 1200 }"
              row-key="id"
              :loading="loading"
...
:deep(.module-detail-material-tabs.ant-tabs) {
  flex: 1 1 0;
  min-height: 0;
  display: flex;
  flex-direction: column;
...
:deep(.module-detail-material-tabs .ant-tabs-nav) {
  flex: 0 0 auto;
  margin-bottom: 0;
}

:deep(.module-detail-material-tabs .ant-tabs-content-holder) {
  flex: 1 1 0;
  min-height: 0;
  overflow: hidden;
}

:deep(.module-detail-material-tabs .ant-tabs-content) {
  height: 100%;
  overflow: hidden;
}

:deep(.module-detail-material-tabs .ant-tabs-tabpane) {
  height: 100%;
  min-height: 0;
  overflow-x: hidden;
  overflow-y: auto;
