<template>
  <div class="inner-skin-page">
    <section class="inner-skin-page__form">
      <div class="inner-skin-page__form-header">
        <span class="inner-skin-page__title">设计输入：</span>
    ...
    <section class="inner-skin-page__table">
      <div class="inner-skin-page__table-header">
        <span class="inner-skin-page__title">内蒙皮加强段设计：</span>
        <a-space :size="12">
...
import { assembleInnerSkinModule, regenerateInnerSkinModel } from './FS1_5_1_1_4/assemblyOperations';
import { runInnerSkinDesignCalculation } from './FS1_5_1_1_4/calculation';
import { applyFs151_1_4InitData } from './FS1_5_1_1_4/initData';
import { extractFs151_1_4SaveParamValues, loadFs151_1_4PageParameters } from './FS1_5_1_1_4/loadPageParameters';
import {
  createDefaultFs151_1_4ParameterList,
  getDesignTableRows,
  NUMERIC_REG,
  type Fs151_1_4ParameterItem,
  type InnerSkinDesignRow,
} from './FS1_5_1_1_4/parameterDefaults';
import {
  FORM_LEFT_FIELDS,
  FORM_RIGHT_FIELDS,
  INNER_SKIN_DESIGN_COLUMN_MAP,
  INNER_SKIN_DESIGN_TABLE_COLUMNS,
  type DesignAntColumn,
} from './FS1_5_1_1_4/tableColumns';

defineOptions({ name: 'customizedProcess3-FS1-5-1-1_4' });
...
const props = withDefaults(
  defineProps<{
    ...
    modalFlag?: boolean;
    pageid?: string;
    parameterTempList?: Fs151_1_4ParameterItem[];
  }>(),
  {
    width: 1000,
    modalFlag: false,
...
const tableColumns = INNER_SKIN_DESIGN_TABLE_COLUMNS;
const formLeftFields = FORM_LEFT_FIELDS;
const formRightFields = FORM_RIGHT_FIELDS;

const selectedRowKeys = ref<Key[]>([]);
const selectedRows = ref<InnerSkinDesignRow[]>([]);

function cloneParameterList(source: Fs151_1_4ParameterItem[]): Fs151_1_4ParameterItem[] {
...
function createInitialParameterList(): Fs151_1_4ParameterItem[] {
  if (!props.parameterTempList || props.parameterTempList.length <= 0) {
    return createDefaultFs151_1_4ParameterList(props.pageid);
  }
  return cloneParameterList(props.parameterTempList);
}

const parameterTempList = ref<Fs151_1_4ParameterItem[]>(createInitialParameterList());
...
watch(
  ...
  val => {
    if (val && val.length > 0) {
      ...
      applyFs151_1_4InitData(parameterTempList.value);
    }
  },
  { deep: true },
);
...
const rowSelection = computed(() => ({
  ...
  onChange: (keys: Key[], rows: InnerSkinDesignRow[]) => {
...
function resolveColumn(column: { dataIndex?: string | number }): DesignAntColumn | undefined {
  return INNER_SKIN_DESIGN_COLUMN_MAP.get(String(column.dataIndex ?? ''));
}

function tableRowKey(record: InnerSkinDesignRow, index?: number) {
...
function handleNumberBlur(record: InnerSkinDesignRow, field: string, event: FocusEvent) {
...
function handleInitData() {
  const ok = applyFs151_1_4InitData(parameterTempList.value);
  parameterTempList.value = [...parameterTempList.value];
  if (!ok) {
    message.warning('未能从流程上下文读取数据，请确认前置页面 FS1-5-1_5_2 已保存');
    return;
  }
  setSaveBtnEnable();
}
...
function handleCalculation() {
  runInnerSkinDesignCalculation(parameterTempList.value);
  parameterTempList.value = [...parameterTempList.value];
  setSaveBtnEnable();
}

function getSingleSelectedRow(): InnerSkinDesignRow | null {
...
async function handleAssembleModule() {
  ...
  const result = await assembleInnerSkinModule(instance, row);
  if (!result.ok) {
...
async function handleRegenModel() {
  ...
  const result = await regenerateInnerSkinModel(row);
  if (!result.ok) {
    if (result.message?.includes('模型号')) message.warning(result.message);
    else if (result.message) message.error(result.message);
    else message.info(result.message ?? '操作失败');
...
async function loadPageParametersIfNeeded() {
  ...
  if (!pageId) return;
  parameterTempList.value = await loadFs151_1_4PageParameters(pageId);
}
...
function getCurrentSaveParamValues() {
  return extractFs151_1_4SaveParamValues(parameterTempList.value);
}
...
.inner-skin-page__title {
  font-size: 15px;
  font-weight: 600;
}

.inner-skin-page__form-header,
.inner-skin-page__table-header {
  display: flex;
  align-items: center;
  gap: 20px;
  margin-bottom: 12px;
...
.inner-skin-page__form-grid {
  display: flex;
  gap: 60px;
  padding: 20px 15px 0;
}

.inner-skin-page__form-col {
  flex: 1;
  min-width: 280px;
}

.inner-skin-page__table {
  margin-top: 16px;
}
...
.inner-skin-table :deep(.ant-table) {
  font-size: 12px;
}

.inner-skin-table :deep(.ant-table-thead > tr > th) {
  padding: 6px 8px;
  text-align: left;
  background: #fafafa;
}

.inner-skin-table :deep(.ant-table-tbody > tr > td) {
  padding: 4px 8px;
  text-align: left;
}
