...
import { createDefaultPage5ParameterList, type Page5ParameterItem } from './parameterDefaults';

export type { Page5ParameterItem };
...
async function applyActivityParameterIds(pageId: string, list: Page5ParameterItem[]): Promise<Page5ParameterItem[]> {
...
export async function loadPage5PageParameters(
  pageId: string,
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Promise<Page5ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultPage5ParameterList(pageKey);
