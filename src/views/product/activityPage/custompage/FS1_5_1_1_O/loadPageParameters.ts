...
import { createDefaultFs151_1_1OParameterList, type Fs151_1_1OParameterItem } from './parameterDefaults';

export type { Fs151_1_1OParameterItem };

async function applyActivityParameterIds(pageId: string, list: Fs151_1_1OParameterItem[]): Promise<Fs151_1_1OParameterItem[]> {
...
export async function loadFs151_1_1OPageParameters(pageId: string): Promise<Fs151_1_1OParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultFs151_1_1OParameterList(pageKey);
  list = await applyActivityParameterIds(pageKey, list);
  return list;
}

export function extractFs151_1_1OSaveParamValues(list: Fs151_1_1OParameterItem[]) {
