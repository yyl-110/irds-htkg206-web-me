...
import { createDefaultFs151JParameterList, type Fs151JParameterItem } from './parameterDefaults';

export type { Fs151JParameterItem };

async function applyActivityParameterIds(pageId: string, list: Fs151JParameterItem[]): Promise<Fs151JParameterItem[]> {
...
export async function loadFs151JPageParameters(pageId: string): Promise<Fs151JParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultFs151JParameterList(pageKey);
