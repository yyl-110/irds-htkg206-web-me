        ...
        <div style="width: 100%; padding: 0 10px; min-height: 530px">
          <a-table
            ...
            <template #bodyCell="{ column, record }">
              <template v-if="isProcessDeviceField(column.dataIndex, processCount, PAGE6_VARIANT)">
                <a-select
              ...
              <template v-else-if="isProcessTimeField(column.dataIndex, processCount, PAGE6_VARIANT)">
                <a-input
                  v-model:value="record[String(column.dataIndex)]"
...
import { runProcessCalculation } from './Process7-page6/calculation';
import {
...
} from './Process7-page6/dataOperations';
import {
  cloneParameterList,
  initCustomizedProcessPage7Data6,
  type Page6ParameterItem,
} from './Process7-page6/parameterDefaults';
import {
  IF_SHOW_OPTIONS,
  buildTableColumns,
  ...
  isProcessTimeField,
  tableRowKey,
} from './Process7-page6/tableColumns';
import type { DeletableProcessColumn, ProcessListItem } from './Process7-page6/models';
import { PAGE6_VARIANT } from './Process7-page6/models';

defineOptions({ name: 'customizedProcess7-page6' });
...
function createInitialParameterList(): Page6ParameterItem[] {
  if (!props.parameterTempList?.length) {
    return initCustomizedProcessPage7Data6(props.pageid);
  }
...
const tableColumns = computed(() => buildTableColumns(processCount.value, PAGE6_VARIANT, tableData.value));
...
function runCalculation() {
  runProcessCalculation(parameterTempList.value, processList.value, '0.9');
  setSaveBtnEnable();
}
...
.page6-table :deep(.ant-table-cell) {
