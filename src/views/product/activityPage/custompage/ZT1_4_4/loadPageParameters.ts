...
import { createDefaultZt1_44ParameterList, type Zt1_44ParameterItem } from './parameterDefaults';

export type { Zt1_44ParameterItem };

async function applyActivityParameterIds(pageId: string, list: Zt1_44ParameterItem[]): Promise<Zt1_44ParameterItem[]> {
...
export async function loadZt1_44PageParameters(pageId: string): Promise<Zt1_44ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultZt1_44ParameterList(pageKey);
