...
import { createDefaultPage4ParameterList, type Page4ParameterItem } from './parameterDefaults';

export type { Page4ParameterItem };
...
async function applyActivityParameterIds(pageId: string, list: Page4ParameterItem[]): Promise<Page4ParameterItem[]> {
...
export async function loadPage4PageParameters(
  pageId: string,
  saved?: Array<{ paramCode?: string; paramKey?: string; paramValue?: string }> | null,
): Promise<Page4ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultPage4ParameterList(pageKey);
