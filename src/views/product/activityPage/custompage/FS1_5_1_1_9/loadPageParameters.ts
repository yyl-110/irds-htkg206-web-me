...
import { createDefaultFs151_1_9ParameterList, type Fs151_1_9ParameterItem } from './parameterDefaults';

export type { Fs151_1_9ParameterItem };

async function applyActivityParameterIds(pageId: string, list: Fs151_1_9ParameterItem[]): Promise<Fs151_1_9ParameterItem[]> {
...
export async function loadFs151_1_9PageParameters(pageId: string): Promise<Fs151_1_9ParameterItem[]> {
  const pageKey = String(pageId ?? '').trim();
  let list = createDefaultFs151_1_9ParameterList(pageKey);
  list = await applyActivityParameterIds(pageKey, list);
  return list;
}

export function extractFs151_1_9SaveParamValues(list: Fs151_1_9ParameterItem[]) {
