        ...
        <a-form label-align="left" :colon="false">
          <div style="width: 99%; float: left">
            <section style="width: 100%; background-color: #ffffff; padding-top: 20px; margin-left: 15px">
              <div style="width: 20%; height: 100%">{{ DYJG3_CONFIG.label }}：</div>
...
import {
  DYJG3_CONFIG,
  cloneDyjg3ParameterList,
  initCustomizedProcessPage7Data5_Dyjg3,
  type DyjgParameterItem,
} from './Process7-page5-Dyjg3/parameterDefaults';
import { OUTPUT_TABLE_COLUMNS } from './Process7-page5-5/tableColumns';
import type { OutputTableRow } from './Process7-page5-5/types';

defineOptions({ name: 'customizedProcess7-page5-Dyjg3' });
...
function createInitialParameterList(): DyjgParameterItem[] {
  if (!props.parameterTempList?.length) {
    return initCustomizedProcessPage7Data5_Dyjg3(props.pageid);
  }
  return cloneDyjg3ParameterList(props.parameterTempList);
}
...
async function initData() {
  const result = await loadDyjgTableData(parameterTempList.value, {
    taskid: props.taskid,
    pageid: props.pageid,
    tableNum: DYJG3_CONFIG.tableNum,
  });
  if (!result.ok) {
