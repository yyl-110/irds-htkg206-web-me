...
export interface Page11AntColumn {
  title?: string;
  dataIndex?: string;
  key?: string;
  width?: number;
  ...
  fixed?: 'left' | 'right';
  children?: Page11AntColumn[];
}

function leaf(title: string, dataIndex: string, width = SCHEME_LEAF_WIDTH, fixed?: 'left' | 'right'): Page11AntColumn {
  return { ...schemeLeaf(title, dataIndex, width), fixed };
}

export const PAGE11_SCHEME_COLUMNS: Page11AntColumn[] = [
  leaf('组合方案', 'p0', SCHEME_COMBO_WIDTH, 'left'),
  {
    ...
    children: [
      ...
      schemeMetricGroup('舵机额定负载速度', 'p2', '°/S', SCHEME_METRIC_WIDTH),
      schemeMetricGroup('舵机最大空载速度', 'p3', '°/S', SCHEME_METRIC_WIDTH),
    ],
  },
  {
    title: '第一级减速',
    align: 'center',
    children: [leaf('电机齿轮', 'p4', SCHEME_LEAF_WIDTH), leaf('第一级从动齿轮数', 'p5', SCHEME_LONG_TITLE_WIDTH)],
  },
  {
    title: '第二级减速',
    align: 'center',
...
export const PAGE11_TABLE_MIN_WIDTH = sumTableColumnWidths(PAGE11_SCHEME_COLUMNS, { extra: 48 });
