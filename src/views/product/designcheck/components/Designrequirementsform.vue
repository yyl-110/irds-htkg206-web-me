...
const handleModalChange = async (row: any, type: string) => {
  ...
  if (type == 'detail') {
    ...
    visibletitle.value = '需求详情';
  } else {
    Detailsflag.value = false;
    visibletitle.value = '需求确认';
  }
  try {
    loading.value = true;
...
<template>
  ...
  <a-modal style="width: 90%" :style="{ top: '5%' }" v-model:visible="visible" :confirm-loading="$isPending()" :mask-closable="false" :title="visibletitle" @cancel="cancel">
            ...
            <div style="display: flex">
              ...
              <div style="width: 60%">
                <a-form-item label="需求标题" name="title">
                  <a-textarea disabled class="textarea" allow-clear v-model:value="formData.demandTitle" :rows="2" maxlength="30" />
    ...
    <div style="margin-top: 20px">
      ...
      <a-textarea class="textarea" allow-clear v-model:value="implementPlan" :rows="2" />
    </div>
    <template #footer>
