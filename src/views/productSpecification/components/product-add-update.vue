...
import taskManaement from './chirdrenComponents/taskManagement.vue';
import planDeliveryFile from './chirdrenComponents/planDeliveryFileList.vue';
...
const handleModalAddOrUpdate = async (row: any, type: any) => {
  ...
  if (row != undefined && row != '') {
    ...
    formData = reactive({
      id: 0,
      title: '',
      planCode: `PRM-${props.planYear}-${props.planQuarter}`,
      planName: '',
      planYear: props.planYear,
      planQuarter: props.planQuarter,
      initiator: userStore.getUser.id,
...
const activeOnClick = async (newTab: string) => {
  ...
  if (newTab == '2') {
    ...
    nextTick(() => {
      planDeliverylistRef.value?.reloadTaskInfosss(planId.value);
    });
  }
};
const localeA = ref({
...
const onSubmitFormData = async () => {
  ...
  formData.planYear = props.planYear;
  formData.planQuarter = props.planQuarter;
  if (formData.id) {
...
const disabledDateend = (current: any) => {
  if (formData.planStartTime) {
    return current.isBefore(formData.planStartTime, 'day');
  } else {
    return current && current < dayjs().startOf('day');
  }
};
    ...
    <a-modal width="100%" wrapClassName="full-modal" v-model:visible="visible" :mask-closable="false" @cancel="cancel">
      <a-tabs @tab-click="activeOnClick" v-model:activeKey="activeKey" :tabBarStyle="{ position: 'sticky', top: 0, zIndex: 10, background: '#fff', width: '80%' }">
                ...
                <a-row>
                  ...
                  <a-col :span="12">
                    <a-form-item :label="$t('产品规划名称')" name="planName" :rules="[{ required: true, message: `${$t('请输入产品规划名称')}` }]" class="f-item">
                      <a-input style="width: 300px" :disabled="disabledPriview" v-model:value="formData.planName" :placeholder="'请输入产品规划名称'" />
                    </a-form-item>
                  </a-col>
                </a-row>
                <!-- <a-row>
                ...
                <a-row>
                  <a-col :span="18">
                    <a-form-item style="margin-left: 100px" :label="$t('产品规划详细描述')" name="planDescribe" class="f-item">
                      <a-textarea v-model:value="formData.planDescribe" :disabled="disabledPriview" :rows="4" placeholder="请输入产品规划详细描述" />
                    </a-form-item>
                  </a-col>
                </a-row>
                <div class="pagecontent-title">
                    ...
                    <a-table :locale="localeA" style="margin-left: 60px; overflow-y: auto" :columns="columns" :pagination="false" :data-source="uploadData_A">
                      <template #bodyCell="{ column, record }">
                        ...
                        <template v-if="column.dataIndex === 'operation'">
                          <a-button :disabled="disabledPriview" @click="removeFile(record)" type="link">删除</a-button>
                    ...
                    <div class="block">
                      <div class="file-uploaad">
                        <a-upload
                          ...
                          :before-upload="beforeUpload"
                          :disabled="disabledPriview"
                          :custom-request="options => customRequest(options, 2)"
                          :show-upload-list="false">
                          <div style="padding: 10px">
                            <EpcIcon type="icon-shangchuanwenjian1" style="color: #1a71ff; font-size: 30px; position: absolute; left: 45px; top: 20px" />
                            <div style="margin-top: 30px">上传文件</div>
                    ...
                    <div class="block">
                      <div class="file-uploaad">
                        <a-upload
                          ...
                          :disabled="disabledPriview"
                          :before-upload="beforeUpload"
                          :custom-request="options => customRequest(options, 3)"
                          :show-upload-list="false">
                          <div style="padding: 10px">
                            <EpcIcon type="icon-shangchuanwenjian1" style="color: #1a71ff; font-size: 30px; position: absolute; left: 45px; top: 20px" />
                            <div style="margin-top: 30px">上传文件</div>
                    ...
                    <div class="block">
                      <div class="file-uploaad">
                        <a-upload
                          ...
                          :disabled="disabledPriview"
                          :before-upload="beforeUpload"
                          :custom-request="options => customRequest(options, 4)"
                          :show-upload-list="false">
                          <div style="padding: 10px">
                            <EpcIcon type="icon-shangchuanwenjian1" style="color: #1a71ff; font-size: 30px; position: absolute; left: 45px; top: 20px" />
                            <div style="margin-top: 30px">上传文件</div>
        ...
        <a-tab-pane key="3" tab="交付清单">
          <planDeliveryFile ref="planDeliverylistRef" />
        </a-tab-pane>
      </a-tabs>
      <!-- 固定的底部按钮区域 -->
      <template #footer>
        <div class="fixed-footer">
          <a-button v-if="activeKey == '1'" :disabled="disabledPriview" @click="onSubmitFormData" type="primary"
            ><EpcIcon type="icon-baocun" style="font-size: 15px" />保存</a-button
          >
          <a-button @click="cancel" type="primary" style="margin-left: 20px">
            <EpcIcon type="icon-quxiao" style="font-size: 17px" />
            关闭</a-button
          >
        </div>
      </template>
