  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    <el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="100px">
      <el-form-item :label="$t('组名')" prop="name">
        <el-input v-model="formData.name" :placeholder="$t('请输入组名')" />
      </el-form-item>
      <el-form-item :label="$t('描述')">
        <el-input v-model="formData.description" :placeholder="$t('请输入描述')" type="textarea" />
      </el-form-item>
      <el-form-item :label="$t('成员')" prop="userIds">
        <el-select v-model="formData.userIds" multiple :placeholder="$t('请选择成员')">
...
const formRules = reactive({
  name: [{ required: true, message: t('组名不能为空'), trigger: 'blur' }],
  description: [{ required: true, message: t('描述不能为空'), trigger: 'blur' }],
  userIds: [{ required: true, message: t('成员不能为空'), trigger: 'blur' }],
