<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" v-loading="formLoading">
      ...
      <el-form-item :label="$t('表达式')" prop="expression">
        <el-input type="textarea" v-model="formData.expression" :placeholder="$t('请输入表达式')" />
      </el-form-item>
    </el-form>
    <template #footer>
...
const formRules = reactive({
  ...
  expression: [{ required: true, message: t('表达式不能为空'), trigger: 'blur' }],
})
