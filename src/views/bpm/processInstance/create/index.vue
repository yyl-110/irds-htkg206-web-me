<template>
  ...
  <div style="width: 100%; height: 100%; background-color: #fff">
    <template v-if="!selectProcessDefinition">
      <el-input
        v-model="searchName"
        class="!w-50% mb-15px pt-20px pl-20px"
      ...
      <ContentWrap
        ...
        <el-empty class="!py-200px" :image-size="200" :description="$t('没有找到搜索结果')" v-else />
      </ContentWrap>
    </template>
...
const getList = async () => {
  ...
  try {
    ...
    if (processInstanceId?.length > 0) {
      ...
      if (!processInstance) {
        message.error(t('重新发起流程失败，原因：流程实例不存在'))
        return
      }
      ...
      if (!processDefinition) {
        message.error(t('重新发起流程失败，原因：流程定义不存在'))
        return
      }
