    ...
    <el-popover
      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          <el-form-item :label="$t(`${nodeTypeName}意见`)" prop="reason">
            <el-input
              v-model="approveReasonForm.reason"
              :placeholder="$t(`请输入${nodeTypeName}意见`)"
              type="textarea"
              :rows="4" />
          </el-form-item>
          <el-form-item
          ...
          <el-form-item v-if="runningTask.signEnable" :label="$t('签名')" prop="signPicUrl" ref="approveSignFormRef">
            <el-button @click="signRef.open()">{{ $t('点击签名') }}</el-button>
            <el-image
    ...
    <el-popover
      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px">
          <el-form-item :label="$t('审批意见')" prop="reason">
            <el-input v-model="rejectReasonForm.reason" :placeholder="$t('请输入审批意见')" type="textarea" :rows="4" />
          </el-form-item>
          <el-form-item>
            <el-button :disabled="formLoading" type="danger" @click="handleAudit(false, rejectFormRef)">
              {{ getButtonDisplayName(OperationButtonType.REJECT) }}
            </el-button>
            <el-button @click="closePopover('reject', rejectFormRef)"> {{ $t('取消') }} </el-button>
          </el-form-item>
        </el-form>
      </div>
    </el-popover>

    <el-button type="info" @click="handleGoBack" style="margin-left: 0px">
...
const approveReasonRule = computed(() => {
  return {
    ...
    signPicUrl: [{ required: true, message: t('签名不能为空'), trigger: 'change' }],
    nextAssignees: [{ required: true, message: t('审批人不能为空'), trigger: 'blur' }],
  }
})
...
const rejectReasonRule = computed(() => {
  return {
    reason: [{ required: reasonRequire.value, message: t('审批意见不能为空'), trigger: 'blur' }],
  }
})
...
const copyFormRule = reactive<FormRules<typeof copyForm>>({
  copyUserIds: [{ required: true, message: t('抄送人不能为空'), trigger: 'change' }],
})
...
const transferFormRule = reactive<FormRules<typeof transferForm>>({
  assigneeUserId: [{ required: true, message: t('新审批人不能为空'), trigger: 'change' }],
  reason: [{ required: true, message: t('审批意见不能为空'), trigger: 'blur' }],
})
...
const validateNextAssignees = () => {
  ...
  for (const item of nextAssigneesActivityNode.value) {
    if (isEmpty(approveReasonForm.nextAssignees[item.id])) {
      message.warning(t('下一个节点的审批人不能为空!'))
      return false
    }
  }
  return true
