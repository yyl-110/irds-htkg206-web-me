    ...
    <div class="pt-20px">
      <el-table v-loading="loading" :data="list">
        <el-table-column
          ...
          width="180"
        />
        <el-table-column :label="$t('摘要')" prop="processInstance.summary" width="180">
          <template #default="scope">
            <div
              class="flex flex-col"
              v-if="
                scope.row.processInstance.summary && scope.row.processInstance.summary.length > 0
              "
            >
        ...
        <el-table-column align="center" :label="$t('操作')" fixed="right" width="80">
          <template #default="scope">
            <el-button link type="primary" @click="handleAudit(scope.row)">{{ $t('办理') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
