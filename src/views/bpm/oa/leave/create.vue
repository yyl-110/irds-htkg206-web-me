<template>
  <el-row :gutter="20">
    <el-col :span="16">
      <ContentWrap :title="$t('申请信息')">
        <el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="80px">
          <el-form-item :label="$t('请假类型')" prop="type">
            <el-select v-model="formData.type" clearable :placeholder="$t('请选择请假类型')">
              <el-option
          ...
          <el-form-item :label="$t('原因')" prop="reason">
            <el-input v-model="formData.reason" :placeholder="$t('请输入请假原因')" type="textarea" />
          </el-form-item>
          <el-form-item>
    ...
    <!-- 审批相关：流程信息 -->
    <el-col :span="8">
      <ContentWrap :title="$t('审批流程')" :bodyStyle="{ padding: '0 20px 0' }">
...
const formRules = reactive({
  type: [{ required: true, message: t('请假类型不能为空'), trigger: 'blur' }],
  reason: [{ required: true, message: t('请假原因不能为空'), trigger: 'change' }],
  startTime: [{ required: true, message: t('请假开始时间不能为空'), trigger: 'change' }],
  endTime: [{ required: true, message: t('请假结束时间不能为空'), trigger: 'change' }],
})
  ...
  if (startUserSelectTasks.value?.length > 0) {
    for (const userTask of startUserSelectTasks.value) {
      if (
        ...
        return message.warning(t(`请选择${userTask.name}的审批人`))
      }
    }
  }
...
onMounted(async () => {
  ...
  if (!processDefinitionDetail) {
    message.error(t('OA 请假的流程模型未配置，请检查！'))
    return
  }
