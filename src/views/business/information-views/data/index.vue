...
import { UnorderedListOutlined, FundOutlined } from '@ant-design/icons-vue';
...
const rowSelection: TableProps['rowSelection'] = {
  ...
  onChange: (selectedRowKeys: Key[], selectedRows: DataType[]) => {
    ...
    //修改批量操作按钮状态
    btnUsedFlag.value = selectedRowList.value.length <= 0;
  },
};
...
const columns = computed<any>(() => [
  ...
  },
  {
    ...
    key: 'ecuPartNumber',
    width: 160,
    sorter: (a: any, b: any) => a.ecuPartNumber.localeCompare(b.ecuPartNumber),
    sortDirections: ['descend', 'ascend'],
  },
  {
  ...
  },
  {
    ...
    key: 'industry',
    width: 120,
    sorter: (a: any, b: any) => a.series.localeCompare(b.series),
    sortDirections: ['descend', 'ascend'],
  },
  {
  ...
  },
  {
    ...
    key: 'emission',
    width: 80,
    sorter: (a: any, b: any) => a.emission.localeCompare(b.emission),
    sortDirections: ['descend', 'ascend'],
  },
  {
    ...
    key: 'fuelType',
    width: 130,
    sorter: (a: any, b: any) => a.fuelType.localeCompare(b.fuelType),
    sortDirections: ['descend', 'ascend'],
  },
...
const columns2 = computed<any>(() => [
  ...
  },
  {
    ...
    key: 'langDesc',
    width: 80,
    sorter: (a: any, b: any) => a.langDesc.localeCompare(b.langDesc),
    sortDirections: ['descend', 'ascend'],
  },
  {
    ...
    key: 'bizRange',
    width: 100,
    sorter: (a: any, b: any) => a.bizRange.localeCompare(b.bizRange),
    sortDirections: ['descend', 'ascend'],
  },
  {
    title: WeiI18n.t('操作').value,
...
async function getListData() {
  ...
  // requestParams.ecuPartNumber = requestParams.ecuPartNumber; // 查询条件件号
  ...
  requestParams.lang = localStorage.getItem('wei-language') + '';
...
async function downloadFile() {
  selectedRowList.value.forEach((selectedRow: any) => {
    if (selectedRow.fileId != null) {
      ...
      if (selectedRow.name != null && selectedRow.name != '') {
        handleEpcDownload(downLoadItem, `${selectedRow.name} + .${selectedRow.docType}`);
      }
    }
  });
}
...
<template>
  <a-card>
    <a-form ref="formRef" class="form_css" layout="inline" :label-col="labelCol" :wrapper-col="wrapperCol" :model="requestParams">
      ...
      <a-form-item :label="$t('件号')" name="docNumber">
      ...
      <a-form-item v-if="rootKey === '1867472048072781825'" :label="$t('ECU硬件型号')" name="ecuPartModel">
        <a-input v-model:value="requestParams.ecuPartModel" style="width: 220px; text-align: left; margin-top: 5px" :placeholder="$t('请输入')" />
      </a-form-item>
      ...
      <a-form-item style="margin-top: 5px" v-if="pageView == '1'">
        <a-button v-if="isDownloadFlag" type="primary" :disabled="btnUsedFlag" @click="downloadFile()">
          {{ $t('下载') }}
        </a-button>
      </a-form-item>
    </a-form>
    <div class="changeDivStyle">
      <span :class="pageView == '1' ? 'changeDivUnChecked' : 'changeDivChecked'" style="border-radius: 3px 0 0 3px" title="图表视图" @click="changePageView('0')">
        <FundOutlined />
      </span>
      <span :class="pageView == '1' ? 'changeDivChecked' : 'changeDivUnChecked'" style="border-radius: 0 3px 3px 0" title="列表视图" @click="changePageView('1')">
  ...
  <div v-if="initFlag">
    <!--  表格列表 -->
    <a-card style="margin-top: 10px" v-show="pageView == '1'" class="b-body2">
      <a-table
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.dataIndex === 'ecuPartNumber'">
            <a-tooltip placement="topLeft">
              <template #title>
                {{ record.ecuPartNumber }}
              </template>
              <div class="text-overflow">{{ record.ecuPartNumber }}</div>
            </a-tooltip>
          </template>
