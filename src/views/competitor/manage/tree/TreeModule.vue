...
// 初始化数据方法
const initTreeData = async (type?: any, treeIds?: any) => {
  if (treeIds) {
  ...
  loading.value = true;
  try {
    const res = await AdminApiCompetitor.getCompetitorTree({ treeId: treeId.value });
    ...
    if (coptData.value.children && coptData.value.children.length > 0) {
      coptData.value.children.map((v: any) => {
        if (v.id === selectedNode.value.id) {
          ...
          unit.value = v.unit;
          compType.value = v.compType;
          paraType.value = v.paraType;
          optionStr.value = v.optionStr;
        }
      });
      coptData.value.children.map((v: any) => {
        if (v.children && v.children.length > 0) {
          v.children.map((k: any) => {
            if (k.id === selectedNode.value.id) {
              ...
              unit.value = k.unit;
              compType.value = k.compType;
              paraType.value = k.paraType;
              optionStr.value = k.optionStr;
            }
          });
        }
      });
...
function toUpTreeNode() {
  if (selectedNode.value.id) {
    ...
    AdminApiCompetitor.upDownSaveTreeKey(params).then(res => {
  ...
  if (selectedNode.value.id) {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        AdminApiCompetitor.delTreeNode(params).then(res => {
          if (res && res.data.code == 200) {
            initTreeData(true);
...
function confirm() {
  const params = {
    ...
    level: DataTitle.value == '添加数据' ? selectedNode.value.level + 1 : selectedNode.value.level,
    fixedFlag: 1,
    userid: userStore.getUser.id,
    categoryName: nodeName.value,
    unit: unit.value,
    compType: compType.value,
    paraType: paraType.value,
    optionStr: optionStr.value,
    treeId: treeId.value,
  };
  AdminApiCompetitor.competitorTreeAdd(params).then(res => {
    if (res && res.data.code == 200) {
      initTreeData(true);
      AddDialogVisible.value = false;
    }
...
defineExpose({ initTreeData });
</script>

<template>
  <div class="TreeModuleModal" v-dragModal>
    <a-modal title="竞品数据属性管理" v-model:visible="visible" width="28%" :getContainer="customGetContainer" :confirm-loading="$isPending()" @cancel="emit('close')">
      <div class="container">
        <div class="controls-wrap">
          <div class="action">
            <div class="icon" style="margin-left: 0" @click="addTreedata" title="添加数据" v-if="selectedNode?.level != '2'">
              <PlusCircleOutlined />
            </div>
            <div class="icon" :style="{ marginLeft: selectedNode?.level == '2' ? '0' : '40px' }" @click="editTree" title="编辑数据" v-if="selectedNode?.level != '0'">
              <FormOutlined />
            </div>
            <div class="icon" @click="toUpTreeNode" title="上移数据" v-if="selectedNode?.level != '0'">
              <ArrowUpOutlined />
            </div>
            <div class="icon" @click="toDownTreeNode" title="下移数据" v-if="selectedNode?.level != '0'">
              <ArrowDownOutlined />
            </div>
            <div class="icon" @click="delTreedata" title="删除数据" v-if="selectedNode?.fixedFlag != '0' && selectedNode?.level != '0'">
              <DeleteOutlined />
            </div>
          </div>
        </div>
  ...
  <a-modal v-model:visible="AddDialogVisible" :title="DataTitle" width="40%">
    <a-form :label-col="{ style: { width: '120px' } }" label-position="right" label-width="100">
      ...
      <a-form-item label="组件类型" v-if="ifLeaf == '1'">
        <a-select v-model:value="compType" placeholder="请选择">
      ...
      <a-form-item label="单位" v-if="ifLeaf == '1' && compType != '3'">
        <a-input v-model:value="unit"></a-input>
      </a-form-item>
      <a-form-item label="参数类型" v-if="(compType == '1' || compType == '2') && ifLeaf == '1'">
        <a-select v-model:value="paraType" placeholder="请选择">
          <a-select-option value="1">字符串</a-select-option>
          <a-select-option value="2">实数</a-select-option>
        </a-select>
      </a-form-item>
      <a-form-item label="下拉值选项" v-if="compType == '2' && ifLeaf == '1'">
        <a-textarea v-model:value="optionStr" placeholder="请输入" rows="4"></a-textarea>
        <div class="tip">选项格式：选项1,选项2,选项3</div>
      </a-form-item>
    </a-form>
    <template #footer>
