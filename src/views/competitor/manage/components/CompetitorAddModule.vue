...
/** 获取分类数据 */
async function getListData(treeIds: any) {
  ...
  try {
    //加载表格数据
    AdminApiCompetitor.getCompetitorTreeInfoList({ treeId: treeId.value }).then(res => {
...
async function confirm() {
  ...
  AdminApiCompetitor.competitorInfoDataAdd(params).then(res => {
    if (res && res.data.code == 200) {
      //调用父页面刷新数据
...
<template>
  <a-modal title="产品数据添加" class="modelstyle" v-model:visible="visible" width="100%" wrapClassName="full-modal" :confirm-loading="$isPending()" @cancel="emit('close')">
