...
/** 表格 formatter 第二参数：antd、a-table / el-table-column 等均兼容，避免依赖 element-plus 类型 */
export type TableFormatterColumn = Record<string, unknown>

/**
 * 日期快捷选项适用于 el-date-picker / ant-design-vue RangePicker 等
 */
 ...
 * @returns {string} 字符串
 */
export function formatPast2WithMs(ms: number): string {
  const day = Math.floor(ms / (24 * 60 * 60 * 1000))
  const hour = Math.floor(ms / (60 * 60 * 1000) - day * 24)
  const minute = Math.floor(ms / (60 * 1000) - day * 24 * 60 - hour * 60)
  const second = Math.floor(ms / 1000 - day * 24 * 60 * 60 - hour * 60 * 60 - minute * 60)
...
/**
 * 表格列时间 Formatter（antd Vue / Element Plus el-table-column formatter 通用），YYYY-MM-DD HH:mm:ss
 *
 * @param _row 行数据
 * @param _column 列配置（不依赖 element-plus 类型）
 * @param cellValue 单元格值
 */
export function dateFormatter(_row: any, _column: TableFormatterColumn | any, cellValue: any): string {
  return cellValue ? formatDate(cellValue) : ''
}

/**
 * 表格列日期 Formatter（antd / Element Plus 通用），YYYY-MM-DD
 *
 * @param _row 行数据
 * @param _column 列配置
 * @param cellValue 单元格值
 */
export function dateFormatter2(_row: any, _column: TableFormatterColumn | any, cellValue: any): string {
...
/**
 * 将时间戳格式化为日期时间字符串（秒/毫秒均可）
 *
 * @param input 时间戳或可被 dayjs 解析的值
 * @returns 格式化字符串 YYYY-MM-DD HH:mm:ss
 */
export function timestampToDateString(input: unknown): string {
  if (input == null || input === '')
    return ''
  if (typeof input === 'object')
    return dayjs(input as dayjs.ConfigType).format('YYYY-MM-DD HH:mm:ss')
