...
async function saveSimpleFlowModel(simpleModelNode: SimpleFlowNode) {
  if (!simpleModelNode) {
    return
  }
...
// 校验节点设置。 暂时以 showTet 为空 未节点错误配置
function validateNode(node: SimpleFlowNode | undefined, errorNodes: SimpleFlowNode[]) {
  if (node) {
    ...
    if (type === NodeType.USER_TASK_NODE || type === NodeType.COPY_TASK_NODE || type === NodeType.CONDITION_NODE) {
      if (!showText) {
