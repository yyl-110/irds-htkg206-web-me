<template>
  <el-drawer
    ...
    <el-tabs type="border-card" v-model="activeTabName" v-if="approveType === ApproveType.USER">
        ...
        <div>
          <el-form ref="formRef" :model="configForm" label-position="top" :rules="formRules">
            <el-form-item :label="`${nodeTypeName}人设置`" prop="candidateStrategy">
              <el-radio-group v-model="configForm.candidateStrategy" @change="changeCandidateStrategy">
                <el-row>
            ...
            <el-form-item v-if="configForm.candidateStrategy == CandidateStrategy.ROLE" label="指定角色" prop="roleIds">
            ...
            <el-form-item
              ...
              label="流程表达式"
              prop="expression">
              <el-input type="textarea" v-model="configForm.expression" clearable style="width: 100%" />
            </el-form-item>
            ...
            <div v-if="currentNode.type === NodeType.USER_TASK_NODE">
              ...
              <el-form-item label="执行动作" prop="timeoutHandlerType" v-if="configForm.timeoutHandlerEnable">
                <el-radio-group v-model="configForm.timeoutHandlerType" @change="timeoutHandlerTypeChanged">
                  <el-radio-button
      ...
      <el-tab-pane label="操作按钮设置" v-if="currentNode.type === NodeType.USER_TASK_NODE" name="buttons">
...
import { useWatchNode, useNodeName, useFormFieldsPermission, useNodeForm, UserTaskFormType, useDrawer } from '../node'
...
// 操作按钮设置
const { buttonsSetting, btnDisplayNameEdit, changeBtnDisplayName, btnDisplayNameBlurEvent } = useButtonsSetting()
...
const showUserTaskNodeConfig = (node: SimpleFlowNode) => {
  ...
  buttonsSetting.value =
    ...
    (node.type === NodeType.TRANSACTOR_NODE ? TRANSACTOR_DEFAULT_BUTTON_SETTING : DEFAULT_BUTTON_SETTING)
  // 4. 表单字段权限配置
