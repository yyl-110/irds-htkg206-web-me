...
import { BPM_HTTP_REQUEST_PARAM_TYPES, BpmHttpRequestParamTypeEnum } from '../../consts'
    ...
    <div v-for="(item, index) in props.header" :key="index" class="flex pt-2">
      ...
      <div class="mr-2">
        <el-form-item
          ...
          <el-input v-if="item.type === BpmHttpRequestParamTypeEnum.FIXED_VALUE" v-model="item.value" class="w-160px" />
        </el-form-item>
        <el-form-item
          :prop="`${bind}.header.${index}.value`"
          :rules="{
          ...
          <el-select v-if="item.type === BpmHttpRequestParamTypeEnum.FROM_FORM" v-model="item.value" class="w-160px!">
            <el-option
