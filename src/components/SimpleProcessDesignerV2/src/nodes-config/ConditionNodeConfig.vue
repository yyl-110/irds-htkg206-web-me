    ...
    <template #header>
      <div class="config-header">
        ...
        <div v-else class="node-name">
          {{ currentNode.name }} <Icon class="ml-1" icon="ep:edit-pen" :size="16" @click="clickIcon()" />
        </div>
...
const saveConfig = async () => {
  if (!currentNode.value.conditionSetting?.defaultFlow) {
    ...
    currentNode.value.conditionSetting = cloneDeep({
      ...
      conditionExpression:
        condition.value?.conditionType === ConditionType.EXPRESSION ? condition.value?.conditionExpression : undefined,
      conditionGroups:
        condition.value?.conditionType === ConditionType.RULE ? condition.value?.conditionGroups : undefined,
    })
  }
  settingVisible.value = false
  return true
