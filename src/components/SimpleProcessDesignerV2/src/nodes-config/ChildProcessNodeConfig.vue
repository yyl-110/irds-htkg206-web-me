<template>
  <el-drawer :append-to-body="true" v-model="settingVisible" :show-close="false" :size="550" :before-close="saveConfig">
    <template #header>
        ...
        <div>
          <el-form ref="formRef" :model="configForm" label-position="top" :rules="formRules">
            ...
            <el-form-item label="选择子流程" prop="calledProcessDefinitionKey">
              <el-select v-model="configForm.calledProcessDefinitionKey" clearable @change="handleCalledElementChange">
                <el-option
            ...
            <el-form-item label="是否自动跳过子流程发起节点" prop="skipStartUserNode">
              <el-switch v-model="configForm.skipStartUserNode" active-text="跳过" inactive-text="不跳过" />
            </el-form-item>
            <el-form-item label="主→子变量传递" prop="inVariables">
              <div class="flex pt-2" v-for="(item, index) in configForm.inVariables" :key="index">
                ...
                <div class="mr-1 flex items-center">
                  <Icon icon="ep:delete" :size="18" @click="deleteVariable(index, configForm.inVariables)" />
                </div>
              </div>
            ...
            <el-form-item v-if="configForm.async === false" label="子→主变量传递" prop="outVariables">
              <div class="flex pt-2" v-for="(item, index) in configForm.outVariables" :key="index">
                ...
                <div class="mr-1 flex items-center">
                  <Icon icon="ep:delete" :size="18" @click="deleteVariable(index, configForm.outVariables)" />
                </div>
              </div>
            ...
            <el-form-item label="子流程发起人" prop="startUserType">
              <el-radio-group v-model="configForm.startUserType">
                <el-radio v-for="item in CHILD_PROCESS_START_USER_TYPE" :key="item.value" :value="item.value">
                  {{ item.label }}
                </el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item
              ...
              <el-radio-group v-model="configForm.startUserEmptyType">
                <el-radio v-for="item in CHILD_PROCESS_START_USER_EMPTY_TYPE" :key="item.value" :value="item.value">
                  {{ item.label }}
                </el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item v-if="configForm.startUserType === 2" label="发起人表单" prop="startUserFormField">
            ...
            <el-form-item label="启用开关" prop="timeoutEnable">
              <el-switch v-model="configForm.timeoutEnable" active-text="开启" inactive-text="关闭" />
            </el-form-item>
            <div v-if="configForm.timeoutEnable">
              ...
              <el-form-item v-if="configForm.timeoutType === DelayTypeEnum.FIXED_DATE_TIME" prop="dateTime">
                <el-date-picker
                  class="mr-2"
            ...
            <el-form-item label="启用开关" prop="multiInstanceEnable">
              <el-switch v-model="configForm.multiInstanceEnable" active-text="开启" inactive-text="关闭" />
            </el-form-item>
            <div v-if="configForm.multiInstanceEnable">
              <el-form-item prop="sequential">
                <el-switch v-model="configForm.sequential" active-text="串行" inactive-text="并行" />
              </el-form-item>
              <el-form-item prop="approveRatio">
                <el-text>完成比例(%)</el-text>
                <el-input-number class="ml-10px" v-model="configForm.approveRatio" :min="10" :max="100" :step="10" />
              </el-form-item>
              ...
              <el-form-item
                v-if="configForm.multiInstanceSourceType === ChildProcessMultiInstanceSourceTypeEnum.FIXED_QUANTITY">
                <el-input-number v-model="configForm.multiInstanceSource" :min="1" />
              </el-form-item>
              <el-form-item
                v-if="configForm.multiInstanceSourceType === ChildProcessMultiInstanceSourceTypeEnum.NUMBER_FORM">
              ...
              <el-form-item
                v-if="configForm.multiInstanceSourceType === ChildProcessMultiInstanceSourceTypeEnum.MULTIPLE_FORM">
                <el-select class="w-200px!" v-model="configForm.multiInstanceSource">
                  <el-option
...
const formRules = reactive({
  ...
  skipStartUserNode: [{ required: true, message: '是否自动跳过子流程发起节点不能为空', trigger: 'change' }],
  startUserType: [{ required: true, message: '子流程发起人不能为空', trigger: 'change' }],
  startUserEmptyType: [{ required: true, message: '当子流程发起人为空时不能为空', trigger: 'change' }],
...
const saveConfig = async () => {
  ...
  if (currentNode.value.childProcessSetting) {
    ...
    currentNode.value.childProcessSetting.startUserSetting.emptyType = configForm.value.startUserEmptyType
    currentNode.value.childProcessSetting.startUserSetting.formField = configForm.value.startUserFormField
    // 7. 超时设置
    ...
    if (configForm.value.timeoutEnable) {
      ...
      if (configForm.value.timeoutType === DelayTypeEnum.FIXED_DATE_TIME) {
        currentNode.value.childProcessSetting.timeoutSetting.timeExpression = configForm.value.dateTime
      }
    }
    // 8. 多实例设置
    ...
    if (configForm.value.multiInstanceEnable) {
      ...
      currentNode.value.childProcessSetting.multiInstanceSetting.approveRatio = configForm.value.approveRatio
      currentNode.value.childProcessSetting.multiInstanceSetting.sourceType = configForm.value.multiInstanceSourceType
      currentNode.value.childProcessSetting.multiInstanceSetting.source = configForm.value.multiInstanceSource
    }
  }
...
const showChildProcessNodeConfig = (node: SimpleFlowNode) => {
  ...
  if (node.childProcessSetting) {
    ...
    // 2. 调用流程
    configForm.value.calledProcessDefinitionKey = node.childProcessSetting?.calledProcessDefinitionKey
    // 3. 是否跳过发起人
    ...
    configForm.value.startUserEmptyType =
      node.childProcessSetting.startUserSetting.emptyType ?? ChildProcessStartUserEmptyTypeEnum.MAIN_PROCESS_START_USER
    ...
    if (configForm.value.timeoutEnable) {
      configForm.value.timeoutType = node.childProcessSetting.timeoutSetting.type ?? DelayTypeEnum.FIXED_TIME_DURATION
      // 固定时长
      if (configForm.value.timeoutType === DelayTypeEnum.FIXED_TIME_DURATION) {
    ...
    // 8. 多实例设置
    configForm.value.multiInstanceEnable = node.childProcessSetting.multiInstanceSetting.enable ?? false
    if (configForm.value.multiInstanceEnable) {
      configForm.value.sequential = node.childProcessSetting.multiInstanceSetting.sequential ?? false
      configForm.value.approveRatio = node.childProcessSetting.multiInstanceSetting.approveRatio ?? 100
...
const loadFormInfo = async () => {
  const childInfo = childProcessOptions.value.find(option => option.key === configForm.value.calledProcessDefinitionKey)
