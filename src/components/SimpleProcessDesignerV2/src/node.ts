...
// import type * as PostApi from '@/api/system/post'
import type * as UserApi from '@/api/system/user'
import type * as UserGroupApi from '@/api/bpm/userGroup'
...
export function useNodeForm(nodeType: NodeType) {
  const roleOptions = inject<Ref<[]>>('roleList', ref([])) // 角色列表
  ...
  const deptOptions = inject<Ref<[]>>('deptList', ref([])) // 部门列表
  ...
  const getShowText = (): string => {
    ...
    // 发起人的部门负责人
    if (configForm.value?.candidateStrategy === CandidateStrategy.START_USER_MULTI_LEVEL_DEPT_LEADER) {
      showText = `发起人连续部门负责人`
    }
    // 流程表达式
  ...
  const handleCandidateParam = () => {
    let candidateParam: undefined | string
    if (!configForm.value) {
  ...
  /**
   *  解析候选人参数
   * @param candidateStrategy
   * @param candidateParam
   */
  const parseCandidateParam = (candidateStrategy: CandidateStrategy, candidateParam: string | undefined) => {
      ...
      return `(${item.rules
        .map(rule => {
          if (rule.leftSide && rule.rightSide) {
            return `${getFormFieldTitle(fieldOptions, rule.leftSide)} ${getOpName(rule.opCode)} ${rule.rightSide}`
          } else {
            // 有一条规则不完善。提示错误
 ...
 * @param field
 */
function getFormFieldTitle(fieldOptions: Array<Record<string, any>>, field: string) {
