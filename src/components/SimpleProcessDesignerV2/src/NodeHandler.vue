...
function addNode(type: number) {
  ...
  if (
    ...
    [NodeType.CONDITION_BRANCH_NODE, NodeType.INCLUSIVE_BRANCH_NODE].includes(props.currentNode?.type)
  ) {
...
<template>
  <div class="node-handler-wrapper">
    <div class="node-handler">
      <el-popover v-if="!readonly" v-model:visible="popoverShow" trigger="hover" placement="right-start" width="auto">
