...
// 用于删除节点
function handleModelValueUpdate(updateValue) {
  emits('update:flowNode', updateValue)
}

function findFromParentNode(nodeList: SimpleFlowNode[], nodeType: number) {
  emits('find:recursiveFindParentNode', nodeList, props.parentNode, nodeType)
}

// 递归从父节点中查询匹配的节点
function recursiveFindParentNode(nodeList: SimpleFlowNode[], findNode: SimpleFlowNode, nodeType: number) {
  if (!findNode) {
    return
  }
...
<template>
  <!-- 发起人节点 -->
  <StartUserNode v-if="currentNode && currentNode.type === NodeType.START_USER_NODE" :flow-node="currentNode" />
  <!-- 审批节点 -->
  <UserTaskNode
    v-if="
      currentNode && (currentNode.type === NodeType.USER_TASK_NODE || currentNode.type === NodeType.TRANSACTOR_NODE)
    "
    :flow-node="currentNode"
    @update:flow-e="handleModelValueUpdate"
