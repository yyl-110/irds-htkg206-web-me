<template>
  <div class="panel-tab__content">
    ...
    <!-- 完成条件配置 -->
    <el-form-item
      v-if="type === 'UserTask' && approveMethod && approveMethod !== ApproveMethodType.RANDOM_SELECT_ONE_APPROVE"
      label="完成条件"
    ...
    >
      <el-input
        ...
        type="textarea"
        :rows="2"
        placeholder="请输入完成条件表达式，例如：${ nrOfCompletedInstances >= nrOfInstances }"
        @change="onCompletionConditionChange"
      />
      <div style="font-size: 12px; color: #666; margin-top: 4px;">
        常用表达式：
        <br/>• ${ nrOfCompletedInstances >= nrOfInstances } - 全部完成（会签）
        <br/>• ${ nrOfCompletedInstances > 0 } - 至少一个完成（或签）
        <br/>• ${ nrOfCompletedInstances/nrOfInstances >= 0.5 } - 至少50%完成
        <br/>• ${ nrOfCompletedInstances/nrOfInstances >= 0.8 } - 至少80%完成
        <br/>• ${ nrOfCompletedInstances >= 2 } - 至少2人完成
      </div>
    </el-form-item>
...
const getElementLoopNew = () => {
  if (props.type === 'UserTask' && bpmnElement.value) {
    ...
    // 读取现有的完成条件
    const loopCharacteristics = bpmnElement.value.businessObject?.loopCharacteristics
    if (loopCharacteristics?.completionCondition?.body) {
...
const onCompletionConditionChange = () => {
  updateLoopCharacteristics()
}
const updateLoopCharacteristics = () => {
  ...
  if (approveMethod.value === ApproveMethodType.RANDOM_SELECT_ONE_APPROVE) {
    ...
    if (approveMethod.value === ApproveMethodType.APPROVE_BY_RATIO) {
      ...
      // 使用自定义完成条件，如果没有则使用默认的会签条件（全部完成）
      const condition = completionCondition.value || '${ nrOfCompletedInstances >= nrOfInstances }'
    ...
    if (approveMethod.value === ApproveMethodType.ANY_APPROVE) {
      ...
      // 使用自定义完成条件，如果没有则使用默认的或签条件
      const condition = completionCondition.value || '${ nrOfCompletedInstances > 0 }'
      if (multiLoopInstance.value) {
        multiLoopInstance.value.completionCondition = bpmnInstances().moddle.create(
          'bpmn:FormalExpression',
          { body: condition }
